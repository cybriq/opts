		...
		if options[i].Name() == "datadir" {
			...
			if _, e = options[i].ReadInput(optVals[i]); !E.Chk(e) {
				...
				if c.ConfigFile.V() == c.ConfigFile.Def {
					e = c.ConfigFile.Set(filepath.Join(datadir, "_prototype.json"))
				}
...
// loadEnvironment scans the environment variables for values relevant to _prototype
...
func (c *Config) GetHelp(hf func(ifc interface{}) error) {
	helpCommand := cmds.Command{
		Name:       "help",
		Title:      "prints information about how to use _prototype",
		Entrypoint: hf,
		Commands:   nil,
	}
