...
//
// // Config defines the configuration options for _prototype. See loadConfig for details on the configuration load process.
// type Config struct {
...
// // createDefaultConfig copies the file sample-_prototype.
...
// The above results in _prototype functioning properly without any config settings while still allowing the user to override settings with config files and command line options.  Command line options always take precedence.
func loadConfig() (
	*Config, []string,
	error,
) {
